...
from mothmonitor import s3_storage
import datetime
import io

def make_boto_mock(mocker):
    c = mocker.patch("boto3.client")
    c("s3").exceptions.NoSuchKey = Exception
...
def test_datasets_list_from_s3(admin_client, mocker):
    ...
    assert len(nights) == 1
    assert nights[0].device.name == "one"
...
def test_datasets_list_from_s3_by_db_match_by_subdir(admin_client, mocker, device):
    c = make_boto_mock(mocker)
    c.return_value.list_objects.side_effect = [
        {"CommonPrefixes": [{"Prefix": "one/"}]},
        {"CommonPrefixes": [{"Prefix": "2025-12-31/"}]},
...
def test_datasets_list_from_s3_by_db_match_by_subdir_again(admin_client, mocker, device):
    c = make_boto_mock(mocker)
    c.return_value.list_objects.side_effect = [
        {"CommonPrefixes": [{"Prefix": "one/"}]},
        {"CommonPrefixes": [{"Prefix": "2025-12-31/"}]},
    ...
    s3_storage.refresh_nights(forced_refresh=True);
    s3_storage.refresh_nights(forced_refresh=True);
    res = admin_client.get('/datasets/list')
